...
class CERTLangChainHandler(BaseCallbackHandler):
    ...
    def on_llm_start(
        ...
        **kwargs: Any,
        ...
        if not model_name:
            serialized_id = serialized.get("id", ["unknown"])
            model_name = serialized_id[-1] if serialized_id else "unknown"

        run.model = model_name

        # Infer provider from model name and serialized
    ...
    def on_tool_end(
        ...
        **kwargs: Any,
        ...
        # Serialize output
        tool_output: Union[Dict[Any, Any], List[Any], str]
